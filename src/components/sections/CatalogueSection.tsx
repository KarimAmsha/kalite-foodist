              ...
              <Reveal key={c.key} delay={i * 0.07}>
                <Card glass className="flex h-full flex-col p-6">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="font-heading text-xl font-bold text-kalite-brown">{c.name}</h3>
                      {c.spotlight && (
                        <span className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-kalite-gold/15 px-3 py-1 text-xs font-semibold text-kalite-gold">
                          <SparkleIcon className="h-3.5 w-3.5" /> {t('spotlight')}
                        </span>
                      )}
                    </div>
                    <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-kalite-red/10 text-kalite-red">
